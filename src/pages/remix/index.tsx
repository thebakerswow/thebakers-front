...
import { RunsDataGrid } from '../bookings-na/runs-data-grid'
...
export function RemixPage() {
  ...
  const copyRunsToClipboard = () => {
    ...
    const formattedRuns = filteredRows.map((run) => ({
      ...
      raidLeader:
        run.raidLeaders?.map((leader) => `${leader.idDiscord};${leader.username}`) || [],
      loot: run.loot,
  ...
  const fetchRuns = async (isUserRequest: boolean) => {
    ...
    try {
      ...
      setRows(
        (data || [])
          .filter((run: any) => run.runType === 'Remix')
          .map((run: any) => ({
            ...run,
  ...
  return (
    <div className='flex min-h-screen w-full flex-col items-center overflow-auto pb-20'>
      ...
      <div className='mx-auto mt-6 flex w-[90%] flex-col p-4'>
          ...
          <div className='flex flex-wrap items-end gap-4'>
            ...
            <div className='flex flex-col'>
              ...
              <Select
                ...
                id='loot-filter'
                value={selectedLoot}
                onChange={(e: SelectChangeEvent) => setSelectedLoot(e.target.value)}
                variant='outlined'
              >
                <MenuItem value='All'>All</MenuItem>
        ...
        {isBulkAddOpen && (
          <Dialog open={isBulkAddOpen} onClose={handleCloseBulkAddDialog} fullWidth maxWidth='md'>
            <DialogTitle className='relative'>
              Add Multiple Runs
              <IconButton aria-label='close' onClick={handleCloseBulkAddDialog} sx={{ position: 'absolute', right: 8, top: 8 }}>
                <CloseIcon />
              </IconButton>
            </DialogTitle>
            <DialogContent>
